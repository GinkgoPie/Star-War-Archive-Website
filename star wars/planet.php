<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="description" content="Explore the captivating Star Wars universe with our dynamic collection of films. Immerse yourself in the galaxy far, far away as you browse through an array of movie titles, each offering a unique adventure. Discover characters, planets, and species, and embark on an intergalactic journey. Join us for an unforgettable cinematic experience.">
    <title>Star Wars - Planet</title>
    <script src="js/script.js"></script>
    <script src="js/people_script.js"></script>
    <link rel="stylesheet" href="css/style.css" />
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Pixelify+Sans">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Quicksand">
</head>
<body class="planet-page-body">
<?php
session_start();
require_once 'navbar.php';
require_once 'database.php';

$planet_id = $_GET['planet_id'];
$planet = getAPlanet($planet_id);
if (!$planet) {
    require_once 'pageNotFound.php';
} else {

    $planet_rotation_period = $planet['planet_rotation_period'];
    $planet_orbital_period = $planet['planet_orbital_period'];
    $planet_diameter = $planet['planet_diameter'];
    $planet_gravity = $planet['planet_gravity'];
    $planet_surface_water = $planet['planet_surface_water'];
    $planet_population = $planet['planet_population'];
    $climate = getClimateForPlanet($planet_id);
    $terrains = getTerrainsForPlanet($planet_id);
    $films = getFilmsForPlanet($planet_id);
    $people = getPeopleForPlanet($planet_id);
    ?>

    <div class="col-md-6">
        <img src="<?php echo rtrim($planet['image_url'], "/revision/latest"); ?>" onerror="this.src='img/default_image.jpg'" style="width:90%" alt="<?php echo $planet['planet_name']; ?>">
        <div class="row">
            <h2 style="margin-top: 5%;margin-left: 2%;;color: darkgoldenrod; text-align: start; font-family: 'Quicksand', sans-serif">Information</h2>
        </div>

        <div class="row">
            <div class="col text-white" >
                <h5>Rotation Period :</h5>
                <h5>Orbital Period :</h5>
                <h5>Diameter :</h5>
                <h5>Gravity :</h5>
                <h5>Surface water :</h5>
                <h5>Population :</h5>
                <h5>Climate :</h5>

            </div>
            <div class="col text-white" >
                <h5><?php echo $planet_rotation_period?></h5>
                <h5><?php echo $planet_orbital_period?></h5>
                <h5><?php echo $planet_diameter?></h5>
                <h5><?php echo $planet_gravity?></h5>
                <h5><?php echo $planet_surface_water?></h5>
                <h5><?php echo $planet_population?></h5>
                <h5><?php echo $climate[0]['planet_climate']?></h5>
            </div>
        </div>
    </div>

    <div class="col-md-6" >
        <h1 style="color: white; text-align: center"><?php echo $planet['planet_name']; ?></h1>

        <?php
        if (count($films) > 0) {
            echo '<p style="margin-top: 6%; margin-left: 2%; color: darkgoldenrod; text-align: start; font-size: x-large; font-family: \'Quicksand\', sans-serif">Appears in</p>';
        }
        ?>

        <div class="row" style="width: 100%" id="films-container">
            <?php
            foreach ($films as $film) {
                ?>
                <div class="col-md-4">
                    <a href="film.php?film_id=<?php echo $film['film_id']; ?>" class="card mb-sm-3" style="background-color: darkgoldenrod; height: 200px; width: 200px">
                        <div class="card-header-tabs text-center text-dark"><?php echo $film['film_title']; ?></div>
                        <div class="card-body text-primary d-flex justify-content-center">
                            <img src="<?php echo $film['image_url']; ?>" onerror="this.src='img/default_image.jpg'" style="max-width:100%; max-height: 100%" alt="<?php echo $film['film_title']; ?>">
                        </div>
                    </a>
                </div>
                <?php
            }
            ?>
        </div>


        <?php
        if (count($people) > 0) {
            echo '<p style="margin-top: 6%; margin-left: 2%; color: darkgoldenrod; text-align: start; font-size: x-large; font-family: \'Quicksand\', sans-serif">People of this planet</p>';
        }
        ?>


        <div class="row">
            <?php
            foreach ($people as $person) {
                ?>
                <div class="col-md-4">
                    <a href="person.php?person_id=<?php echo $person['peopleID']; ?>" class="card mb-sm-3" style="background-color: darkgoldenrod; height: 200px; width: 250px">
                        <div class="card-header-tabs text-center text-dark"><?php echo $person['people_name']; ?></div>
                        <div class="card-body text-primary d-flex justify-content-center">
                            <img src="<?php echo rtrim($person['image_url'], "/revision/latest"); ?>" onerror="this.src='img/default_image.jpg'" style="max-width:100%" alt="<?php echo $person['people_name']; ?>">
                        </div>
                    </a>
                </div>
                <?php

            }
            ?>
        </div>


        <?php
        if (count($terrains) > 0) {
            echo '<p style="margin-top: 6%; margin-left: 2%; color: darkgoldenrod; text-align: start; font-size: x-large; font-family: \'Quicksand\', sans-serif">Terrain</p>';
        }
        ?>


        <div class="row">
            <?php
            foreach ($terrains as $terrain) {
                ?>
                <div class="col-md-4">
                    <div class="card mb-sm-3" style="background-color: darkgoldenrod; height: 200px; width: 250px">
                        <div class="card-header-tabs text-center text-dark"><?php echo $terrain['planet_terrain']; ?></div>
                        <div class="card-body text-primary d-flex justify-content-center">
                            <img src="<?php echo rtrim($terrain['image_url'], "/revision/latest"); ?>" onerror="this.src='img/default_image.jpg'" style="max-width:100%" alt="<?php echo $terrain['planet_terrain']; ?>">
                        </div>
                    </div>
                </div>
                <?php

            }
            ?>

        </div>
    </div>






    <?php
}
?>

<script src="https://code.jquery.com/jquery-3.5.1.slim.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.11.6/dist/umd/popper.min.js"></script>
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
</body>
</html>

